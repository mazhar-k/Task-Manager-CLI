...
Changes in add command:

2. .\task add 2"hello world" instead of .\task add 2 "hello world" -> remove space between priority number and task string to accept the string as part of an arguement


Changes in task.test.js for running tests:

1. `deL` instead of `del` 
-> del is a keyword in python and hence cannot be used as a name for an user defined function.

2. Remove `expect.stringContaining()` from certain tests 
-> expect.stringContaining added text "StringContaining" to the expected output and hence fails the tests even for the correct output.

3. 'execSync(tasksTxtCli("add", `${i}"${tasks}"`)));' instead of 'execSync(tasksTxtCli("add", `${i} "${tasks}"`)));' for all instances
-> As the add command is changed, the command needs to be changed for all test cases as well
